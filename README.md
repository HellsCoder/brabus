...
## Обработка кликов
Чтобы обработать клик по элименту у элимента должен быть присвоен класс b-fx, а так же присутствовать аттрибут fx
```html
<!--divClick должен быть присвоен при старте Brabus
brabus.run({
    divClick: (e) => {
      e - элимент по которому произведен клик
    }
});
-->
<div class="styled-div b-fx" fx="divClick"></div>
